...
type APPListInput struct {
	Info     string `json:"info"    form:"info" comment:"查找信息" validate:""`                          //关键词
	PageNo   int    `json:"page_no" form:"page_no" comment:"页数" validate:"required,min=1,max=999"`     //页数
	PageSize int    `json:"page_size" form:"page_size" comment:"页码" validate:"required,min=1,max=999"` //每页条数
}

func (param *APPListInput) GetValidParams(c *gin.Context) error {
...
func (param *APPDetailInput) GetValidParams(c *gin.Context) error {
	return public.DefaultGetValidParams(c, param)
}

type StatisticsOutput struct {
	Today     []int64 `json:"today"   form:"today"     comment:"今日统计" example:"" validate:"required"`
	Yesterday []int64 `json:"yesterday" form:"yesterday" comment:"昨日统计" example:"" validate:"required"`
}
