...
type ServiceDeleteInput struct {
	ID int64 `json:"id" form:"id" comment:"服务ID" example:"56" validate:"required"` // 服务 ID
}
...
type ServiceAddHTTPInput struct {
	ServiceName string `json:"service_name" form:"service_name" commnet:"服务名"  example:"" validate:"required,valid_service_name"`
	ServiceDesc string `json:"service_desc" form:"service_desc" commnet:"服务描述" example:"" validate:"required,max=255,min=1"`

	RuleType       int    `json:"rule_type" form:"rule_type"  commnet:"接入类型"  example:"" validate:"max=1,min=0"`
	Rule           string `json:"rule"   form:"rule"  commnet:"接入路径:域名或前缀" example:"" validate:"required,valid_rule"`
	NeedHttps      int    `json:"need_https"      form:"need_https"  commnet:"支持 https"  example:"" validate:"max=1,min=0"`
	NeedStripUri   int    `json:"need_strip_uri"  form:"need_strip_uri"  commnet:"启用 strip_uri" example:"" validate:"max=1,min=0"`
	NeedWebsocket  int    `json:"need_websocket"  form:"need_websocket"  commnet:"是否支持 websocket" example:"" validate:"max=1,min=0"`
	UrlRewrite     string `json:"url_rewrite"   form:"url_rewrite"  commnet:"url 重写功能" example:"" validate:"valid_url_rewrite"`
	HeaderTransfor string `json:"header_transfor" form:"header_transfor" commnet:"header 转换" example:"" validate:"valid_header_transfor"`

	OpenAuth          int    `json:"open_auth" form:"open_auth" commnet:"是否开启权限" example:"" validate:"max=1"`
	BlackList         string `json:"black_list" form:"black_list" commnet:"黑名单 ip" example:"" validate:""`
	WhiteList         string `json:"white_list" form:"white_list" commnet:"白名单 ip" example:"" validate:""`
	ClientipFlowLimit int    `json:"clientip_flow_limit" form:"clientip_flow_limit" commnet:"BS 客户端 ip 限流" example:"" validate:"min=0"`
	ServiceFlowLimit  int    `json:"service_flow_limit"  form:"service_flow_limit" commnet:"服务端限流"  example:"" validate:"min=0"`

	RoundType              int    `json:"round_type" form:"round_type" commnet:"轮询方式"  example:"" validate:"max=3,min=0"`
	IpList                 string `json:"ip_list" form:"ip_list" commnet:"ip 列表" example:"" validate:"required,valid_ipportlist"`
	WeightList             string `json:"weight_list" form:"weight_list" commnet:"BS 权重列表" example:"" validate:"required,valid_weightlist"`
	UpstreamConnectTimeout int    `json:"upstream_connect_timeout" form:"upstream_connect_timeout" commnet:"建立连接超时" example:"" validate:"min=0"`
	UpstreamHeaderTimeout  int    `json:"upstream_header_timeout" form:"upstream_header_timeout" commnet:"获取 header 超时"  example:"" validate:"min=0"`
	UpstreamIdleTimeout    int    `json:"upstream_idle_timeout" form:"upstream_idle_timeout" commnet:"连接最大空闲时间" example:"" validate:"min=0"`
	UpstreamMaxIdle        int    `json:"upstream_max_idle" form:"upstream_max_idle" commnet:"最大空闲连接数" example:"" validate:"min=0"`
}
