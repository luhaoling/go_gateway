...
type HttpRule struct {
	...
	NeedStripUri   int    `json:"need_strip_uri" 	gorm:"column:need-strip_uri"	description:"启用strp_uri 1=启用"`
	UrlRewrite     string `json:"url_rewrite" 		gorm:"column:url_rewrite"		description:"url重写功能,每行一个"`
	HeaderTransfor string `json:"header_transfor"   gorm:"column:header_transfor"	description:"header转化支持增加(add)、删除(del)、修改(edit) 格式:add headname headvalue"`
}
